...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		hEdit = CreateWindowEx(WS_EX_CLIENTEDGE, TEXT("EDIT"), TEXT("2018年6月4日 12時32分14秒"), WS_VISIBLE | WS_CHILD | ES_AUTOHSCROLL, 0, 0, 0, 0, hWnd, 0, ((LPCREATESTRUCT)lParam)->hInstance, 0);
		SendMessage(hEdit, WM_SETFONT, (WPARAM)hFont, 0);
		hButton = CreateWindow(TEXT("BUTTON"), TEXT("文字列をVARIANT型日付時間に変換"), WS_VISIBLE | WS_CHILD, 0, 0, 0, 0, hWnd, (HMENU)IDOK, ((LPCREATESTRUCT)lParam)->hInstance, 0);
	...
	case WM_COMMAND:
		if (LOWORD(wParam) == IDOK)
		{
			...
			double dt = 0.0;
			HRESULT hr = VarDateFromStr(szText, LANG_USER_DEFAULT, 0, &dt);
			if (SUCCEEDED(hr))
			{
				VARIANT varDate;
				VariantInit(&varDate);
